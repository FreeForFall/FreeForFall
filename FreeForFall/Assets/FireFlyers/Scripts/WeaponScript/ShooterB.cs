...
public class ShooterB : MonoBehaviour {
	...
	void Update () {
		if (Input.GetMouseButtonDown(0))
        {
            ...
            projectile_body.AddForce( transform.forward * projectile_force);
        ...
        {
            GameObject temp_projectile;
            temp_projectile = Instantiate(grip, Launcher.transform.position, Launcher.transform.rotation,Launcher.transform.parent) as GameObject;
            Rigidbody projectile_body;
            projectile_body = temp_projectile.GetComponent<Rigidbody>();
            projectile_body.AddForce(transform.forward * projectile_force);
            Destroy(temp_projectile, 10.0f);
